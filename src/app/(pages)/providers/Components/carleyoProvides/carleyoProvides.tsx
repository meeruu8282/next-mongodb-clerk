    ...
    <div className=" flex flex-col h-screen w-auto max-w-[592px] max-h-[481px] ml-5 gap-[32px]  ">
      <div className=" flex flex-col h-auto w-screen max-w-[592px] max-h-[172px] gap-[16px] ">
        <div className=" w-screen h-auto max-w-[592px] max-h-[104px]">
          <h1 className="text-[poppins] sm:text-[20px] md:text-[30px] text-center lg:text-[40px] font-semibold leading-13 tracking-tight lg:text-left text-white">
            {header}
          </h1>
        </div>
        <div className=" w-screen h-auto max-w-[592px] max-h-[96px]">
          <p className="text-[poppins]  md:text-[15px] lg:text-lg text-center font-normal leading-8 tracking-tight lg:text-left text-white opacity-75">
            {paragraph}
          </p>
        </div>
      </div>

      <div className=" flex flex-col justify-center w-screen h-auto max-w-[592px] max-h-[181px] gap-[16px] ">
        <div className=" flex flex-row mx-auto  items-center w-screen h-auto max-w-[592px] max-h-[45px] mt-5 gap-[12px]">
          {/*in components folder */}
          <UnionCheckmark /> {/* mediaquerey is called by style from  carleyoProvides.module.css */}
          <div
            className={` w-screen h-auto max-w-[380px] ${style.textsize}  sm:text-[10px] md:text-[15px] lg:text-[20px]`}
          >
      ...
      <div className=" w-[301px] h-[72px] gap-[24px] mx-auto lg:mx-0">
        <Link href="/providers/auth/signIn" passHref>
          <div className=" rounded-[25px] w-[301px] h-[72px] bottom-0 mx-auto  bg-gradient-to-br from-[rgba(255, 255, 255)] via-[rgba(255, 255, 255)] to-[rgba(255, 255, 255)]  ">
            <CarelyoButton
              className="  w-[301px] h-[72px] p-[22px 32px] rounded-[25px]  border bg-opacity-25 bg-white gap-[12px] font-semibold "
              buttonText={`${addtext}`}
            />
          </div>
        </Link>
