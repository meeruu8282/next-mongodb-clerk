# Payload
 ...
 This code is either from, or based on the website template that is generated by the npx create-payload-app command. In order to create a new collection, look at the index.ts file for Comments, Pages, Posts or Projects for examples and inspiration. In order to restrict access to collections based on authorship and/or role, look at the access hooks for the Blogpost collection for examples. 
 The code for the frontend website that comes with the template has been left in place to serve as an example of how to handle calls to the database and how to show the information that is fetched from it. 
 The files for the Carelyo website can be found in the src/app/(pages)/providers/ folder. 
 Try to keep a title field and a upload field for images in a meta group in a collection, otherwise the generateMeta utilty function in app/_utilities/ cannot be used for that collection 

## Payload 2.0

Payload 2.0 Changelog: https://github.com/payloadcms/payload/releases/tag/v2.0.0
Note that Payload recently got updated to 2.0, so 3rd party resources can be inaccurate. There are some changes that must be accounted for, or the program will just crash on start.

## TODO

 Create a collection for the providers, each item should have a category (as in hospital, clinic, self employed doctor etc), a relationship to a page for the provider (this page can be created and filled with data through the REST api when the user signs up (the graphql api cannot be used to upload media, so it can't be used)), and an admin user for that provider. This user should have access to edit and update the provider's page, and only that page. There is currently a role called "doctor", which could be renamed to fit this purpouse, or a new user type can be added for the same result.
 The current blocks that came with the template may be insufficient, so new ones need to be created in that case.
 The files in the seed folder should be removed, as they only serve to generate the demo website and add some mockdata to the database.
...
- #### Bloposts
    ...
    A variant of posts, created specifically in order to control access, so that only users with the admin or author role can create, update and delete content.
...
- `author`: They can access the Payload admin panel and can perform limited operations based on their user (see below).
- `doctor`: They can access the Payload admin panel and can perform limited operations based on their user (see below), this name should probably be changed to something along the lines of "clinic admin" for clarity.
...
- `blogposts`: Everyone can access published blogposts, but only admins can create, update, or delete them. Authors can create blogposts, as well as update and delete ones that they have created.
- `healthcare facility details`: Everyone can access published pages, but only admins can create, update, or delete them. Doctors can create pages, as well as update and delete ones that they have created.
