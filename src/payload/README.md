...
This code is either from, or based on the website template that is generated by the npx create-payload-app command. In order to create a new collection, look at the index.ts file for Comments, Pages, Posts or Projects for examples and inspiration.
In order to restrict access to collections based on authorship and/or role, look at the access hooks for the Blogpost collection for examples. The code for the frontend website that comes with the template has been left in place to serve as an example of how to handle calls to the database and how to show the information that is fetched from it. The files for the Carelyo website can be found in the src/app/(pages)/providers/ folder.
...
Note that Payload recently got updated to 2.0, so 3rd party resources can be inaccurate. The most important part of this change is where the Rich Text admin property for fields is set.
...
- `doctor`: They can access the Payload admin panel and can perform limited operations based on their user (see below).
